...
- [x] **T23** Verify and lock in that `SSHConnection.connect()`'s quick-retry-and-redeploy cascade runs only for errors that mean SSH reached the host but the helper/handshake misbehaved (`.handshakeTimeout`, `.serverNotResponding`, `.helperStartupTimeout`) and that `.hostUnreachable`, `.connectionRefused`, `.authenticationFailed`, and `.connectionTimeout` throw immediately with no retry and no redeploy. (`.connectionTimeout` means SSH could not even reach the host, so the cascade and the outer jittered retry are pointless — both fast-fail it, fixing the real-world case where a saved host that has gone off the network would otherwise burn ~a minute of stacked 5s timeouts.)
...
A `.connectionTimeout` (SSH could not reach the host, e.g. a saved host that has gone off the current network) is treated as terminal, not transient: it skips the deploy/redeploy cascade in `connect()` and is not retried by `RemoteConnectRetry`. The spec's original framing retried connection timeouts, which in practice stacked three 5s connect timeouts against the redeploy cascade and turned one off-network host into roughly a minute. A reachable-but-wedged host still reaches the cascade through `.handshakeTimeout`/`.serverNotResponding`/`.helperStartupTimeout`.
...
- [x] **T49** `testUnreachableHostFailsFastWithoutRetryStorm` - Build an on-demand state for a guaranteed-unreachable host (a `192.0.2.0/24` TEST-NET address that SYN-times-out, the same failure mode as a saved host gone off the network) and assert `connectIfNeeded` resolves to an inline `.unavailable` state within a few seconds (no retry storm) while keeping cached content visible. The precise error-to-reason mapping is covered deterministically by `RemoteConnectRetryPolicyTests`.
