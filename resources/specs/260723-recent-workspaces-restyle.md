...
- Clicking a row opens it (single click; Enter for the keyboard-selected row unchanged). A remote open shows its progress in the clicked row, a small spinner in place of the dot and "connecting to <server>..." on the second line, and further clicks are ignored until it finishes; failures land as the red warning in the same row. Hovering a row reveals three inline actions: pin/unpin, reveal in Finder (local items only), and remove. The right-click menu keeps its existing entries.
